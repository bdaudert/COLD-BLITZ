...
if __name__ == '__main__':
    ...
    for model in LOCA_CMIP5_MODELS.keys():
        ...
        data_dir = 'RESULTS/loca/' + model + '/'
        for var_name in ['tmin', 'tmax']:
            for rcp in ['rcp45', 'rcp85']:
                data_dir2 = '/media/DataSets/loca/' + model + '/' + rcp + '/'
                slopes_temps, slopes_sums,lons, lats = get_array_data(var_name, rcp, years, data_dir, data_dir2)
                out_file = data_dir + var_name + '_' + rcp + '_temps_slopes.tif'
                array_to_raster(lats, lons, slopes_temps, out_file, nodata=-9999)
                out_file = data_dir + var_name + '_' + rcp + '_sums_slopes.tif'
                array_to_raster(lats, lons, slopes_sums, out_file, nodata=-9999)
